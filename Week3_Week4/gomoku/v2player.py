...
class vvamp_player:
    ...
    def move(self, board, last_move, valid_moves, max_time_to_move=1000):
        ...
        """
        time_deadline = time.time() + (max_time_to_move*0.99/1000) # Deadline is 99% of the time to move in ms in ns. Can be optimized

        # If only 1 move is possible, do that move
        if(len(valid_moves) == 1):
            return valid_moves[0]

        currentGame = gomoku.gomoku_game(19, board, self.play)      # Current game board
